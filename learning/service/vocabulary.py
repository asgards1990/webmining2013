def get_vocabulary():
    return ['unable',
'direct',
'assimilatory','assimilative','assimilating',
'rare',
'battered',
'obliging','complaisant',
'veracious','right',
'wrong','incorrect','faulty',
'unaccustomed',
'unresisting','supine','resistless',
'avirulent',
'understaffed','undermanned','short-staffed','short-handed',
'nonadhesive',
'ungummed',
'adoptable',
'left-handed','heavy-handed','handless','ham-handed','ham-fisted','butterfingered','bungling','bumbling',
'positive','plus',
'disadvantageous',
'negative','minus',
'better','best',
'unaffected',
'negative',
'dismissive',
'acrophobic',
'agoraphobic',
'algophobic',
'claustrophobic',
'hydrophobic','aquaphobic',
'mysophobic',
'numb',
'xenophobic',
'unagitated',
'disagreeable',
'harsh','abrasive',
'dead',
'pulseless','inanimate','breathless',
'lifeless',
'selfless','altruistic',
'katabolic','catabolic',
'umbrageous','outraged','indignant','incensed',
'resentful',
'unbitter',
'nonliving','non-living','inanimate',
'perked up',
'unappetizing','unappetising',
'inappropriate',
'wrong','improper',
'due',
'malapropos',
'out of place','inapposite',
'aphasic',
'voiceless','aphonic',
'ashamed',
'insolent','brazen-faced','brazen','brassy','bodacious','barefaced','bald-faced','audacious',
'uncommitted','unattached',
'attractive',
'piquant','engaging',
'rosy','fortunate',
'unfortunate','inauspicious',
'prosperous','lucky','golden','favourable','favorable',
'ominous','inauspicious','ill',
'thunderous',
'unauthorized','unauthorised',
'unconstitutional',
'unconscious',
'threatening','sinister','ominous','minatory','minacious','menacing','forbidding','baleful',
'shuddery','shivery','scary','scarey','chilling',
'unbarrelled','unbarreled',
'unstratified',
'smooth-haired',
'beautiful',
'sightly','fair','comely','bonny','bonnie',
'gorgeous',
'picturesque',
'pretty',
'ravishing',
'stunning',
'fugly',
'unbanded',
'beneficent',
'benefic',
'maleficent',
'malevolent',
'catty','cattish','bitchy',
'vicious','venomous','poisonous',
'venomed',
'vixenish',
'unmalicious',
'benignant','benign',
'best',
'top-quality','top-grade','high-grade',
'unsurpassable',
'worst',
'better',
'finer',
'worse',
'better',
'reckless','rash','heady','foolhardy',
'unbound',
'untethered',
'unbound',
'plucky','gutsy',
'dyspnoeic','dyspnoeal','dyspneic','dyspneal','breathless',
'subdued','dim',
'subdued','soft',
'undimmed','bright',
'stormy',
'dirty',
'incapable',
'thorough',
'perfunctory','passing','cursory','casual',
'reckless','heedless',
'cautious',
'uncompartmented',
'self-confident','self-assured',
'frozen','fixed',
'unalterable','inalterable',
'cheerful',
'glad','beaming',
'slate-black',
'saturated','pure',
'unchristlike','unchristianly',
'unclassified',
'raw','crude',
'unused','fresh',
'unclean','soiled','dirty',
'augean',
'draggled','bedraggled',
'fouled','befouled',
'smutty','black',
'dirty-faced',
'nasty','foul','filthy',
'squalid','sordid','flyblown',
'oily','greasy',
'muddy','mucky',
'smudgy',
'snotty','snot-nosed',
'unswept',
'unwashed',
'dirty',
'profane','blue','blasphemous',
'smutty','nasty','foul','filthy','cruddy',
'salacious','raunchy','obscene','lewd',
'tref','terefah','nonkosher',
'confused',
'soft','mild','balmy',
'inclement',
'stupid',
'intimate',
'uncousinly',
'fogbound',
'unpublishable',
'fireproof',
'unlit','unlighted',
'comfy','comfortable',
'tough','bad',
'comfortless',
'wretched','miserable',
'warm',
'uncomfortable',
'noncommercial',
'uncommercial',
'comparable to',
'competent',
'workmanlike',
'unworkmanlike',
'competent',
'noncompetitive',
'uncompetitive',
'complaintive','complaining',
'whiny','whiney','querulous','fretful',
'uncommitted',
'thoroughgoing','thorough','exhaustive',
'full-blown',
'afraid',
'solicitous',
'dissonant','discrepant','at variance',
'confined',
'congenial',
'sociable',
'uncongenial','incompatible',
'inconsistent','discrepant',
'unfitting','out or keeping','incompatible','inappropriate',
'unconquerable',
'incompatible','discrepant',
'conspicuous',
'rank','gross','glaring','flagrant','egregious','crying',
'discernible','discernable',
'indiscernible',
'differentiable',
'constant',
'rehabilitative','reconstructive',
'ruinous','blasting',
'cataclysmic','cataclysmal',
'devastating','crushing',
'negative','damaging',
'ravaging',
'discontented','discontent',
'unsatisfied','ungratified','restless',
'unchallengeable',
'unargumentative',
'noncontentious',
'inconvenient',
'awkward',
'unconventional',
'conformist',
'unconformist','nonconformist',
'untraditional','nontraditional',
'unpersuasive',
'raw',
'untoasted',
'reprobate','perverted','perverse','depraved',
'sordid','dirty',
'putrid',
'incorrupt',
'incorruptible',
'unspoiled','uncorrupted',
'antiphlogistic',
'unembodied','unbodied','disembodied','discorporate','bodiless',
'wrong','incorrect',
'erroneous',
'fallacious',
'wrongheaded',
'uncorrected',
'unremedied',
'unregenerate','unreformable',
'unmanageable','uncorrectable','uncontrollable',
'corn-fed',
'constipated',
'courteous',
'well-mannered','mannerly',
'nice','gracious','courteous',
'unparliamentary',
'uncivil','rude',
'overcredulous',
'incredulous',
'censorious',
'uncritical',
'polar','pivotal',
'noncurrent',
'superannuated','outdated','out-of-date',
'execrable','damnable',
'blest','blessed',
'golden','fortunate',
'damaged',
'beaten-up','beat-up','battered',
'knocked-out',
'storm-beaten',
'undatable',
'undated','dateless',
'indecent',
'vulgar','gross','earthy','crude',
'hollywood',
'indelicate',
'undeclared',
'decorous',
'in good taste',
'staid','sedate',
'shallow',
'indefeasible',
'recalcitrant',
'nonresistant',
'definite',
'low-spirited','low','grim','gloomy','downhearted','downcast','down in the mouth','down','dispirited','depressed','blue',
'gladdened','exhilarated',
'infrangible',
'exigent','exacting',
'peremptory',
'importunate',
'absolute',
'undemonstrative',
'undeniable',
'positive','irrefutable','incontrovertible',
'unreliable','undependable',
'dependent',
'helpless',
'neutral',
'undescriptive',
'plummy',
'preferred','preferable',
'burnt-out','burnt','burned-over','burned-out','burned',
'well-kept','maintained','kept up',
'saved','protected',
'vestigial','rudimentary',
'hard','difficult',
'arduous',
'unenviable','sticky','embarrassing','awkward',
'touchy','ticklish','delicate',
'fractious',
'tight','nasty',
'rough','rocky',
'tough','rugged',
'serious',
'tall',
'thorny',
'troublesome',
'infra dig',
'undiplomatic',
'alienating',
'indirect',
'bluff',
'oblique','devious',
'discerning',
'promiscuous',
'discriminating',
'good',
'unselective',
'indistinct',
'muzzy','hazy','fuzzy','foggy','blurry','blurred','bleary',
'indiscrete',
'submissive',
'bowing','bowed',
'wormy','wormlike','grovelling','groveling','cringing',
'henpecked','dominated',
'unsubmissive','unservile',
'untheatrical',
'potable','drinkable',
'incisive',
'metal-cutting',
'salt',
'sparkly','scintillating','frothy','effervescent','bubbling',
'propulsive','propelling','propellent','propellant',
'stagnant','moribund',
'hot',
'worried','apprehensive',
'uneatable','inedible',
'poisonous',
'uneducated',
'defunct',
'sisyphean',
'bounden',
'incumbent on',
'soignee','soigne',
'stoical','stoic',
'verifiable','falsifiable','confirmable',
'speculative','notional',
'out of work','jobless','idle',
'unemployable',
'entranced','enthralled','delighted','charmed','captivated','beguiled',
'disenchanted',
'unburdened','burdenless',
'unburdened',
'exportable',
'unexportable',
'inflated','hyperbolic',
'remittent',
'enlightened',
'unenergetic','slowgoing',
'glowing',
'nostalgic',
'homesick',
'unquestionable',
'primary','basal',
'unessential','inessential',
'nonessential','incidental',
'estimable',
'admirable',
'contemptible',
'scurvy','scummy','miserable','low-down','low','abject',
'pitiful','pitiable','pathetic',
'unethical',
'slighting','depreciatory','depreciative','deprecatory','deprecative','deprecating','belittling',
'disparaging','derogatory','derogative',
'supercilious','snide','sneering',
'offensive','dysphemistic',
'unhappy','dysphoric','distressed',
'patchy',
'unparallel',
'perfect',
'rough','approximative','approximate',
'exchangeable',
'unexchangeable',
'condemning','condemnatory',
'nonexistent',
'nonextant',
'inexpedient',
'expendable',
'unexpendable',
'extricable',
'fingerless',
'inextinguishable',
'untrue',
'two-timing','cheating','adulterous',
'unreliable','undependable',
'unfailing','foolproof',
'unerring','inerrant','inerrable',
'quaint',
'unstylish','unfashionable',
'choosy','choosey',
'unfastidious',
'blubbery',
'favourable','favorable',
'indulgent',
'unfavourable','unfavorable',
'infelicitous',
'inept','inapt','ill-chosen','cumbersome','clumsy','awkward',
'unfortunate',
'barren',
'unimpregnated','unfertilized','unfertilised',
'unhewn',
'fit',
'unfit',
'handicapped','disabled',
'worthy',
'unsuitable',
'stiff','rigid',
'intransigent','inexorable','adamantine','adamant',
'adventive',
'unforgiving',
'fortunate',
'better off',
'happy','felicitous',
'providential','miraculous','heaven-sent',
'lucky',
'unfortunate',
'abject',
'fateful','fatal','disastrous','calamitous','black',
'roofless','homeless','dispossessed',
'wretched','poor','pitiful','pitiable','piteous','pathetic','misfortunate','miserable','hapless',
'unlucky','ill-starred','ill-omened','ill-fated','doomed',
'too bad','regrettable',
'fragrant',
'unpleasant-smelling','stinky','malodourous','malodorous','ill-smelling',
'stinking','smelly','noisome','ill-scented','funky','foul-smelling','foul','foetid','fetid',
'musty','fusty','frowsty',
'niffy',
'putrid-smelling',
'pleasant-smelling',
'unfree',
'addled',
'spoilt','spoiled','bad',
'day-old',
'unfriendly',
'scowling','beetle-browed',
'unneighbourly','unneighborly',
'unfriendly',
'unfruitful',
'sperm-filled',
'nonfunctional',
'nonfunctional','malfunctioning',
'wrong','haywire','awry','amiss',
'defective','bad',
'general-purpose','all-purpose',
'untechnical','nontechnical',
'nonproprietary',
'penurious','parsimonious',
'genuine','echt',
'forged','bad',
'base',
'phony','phoney','fake','bogus','bastard',
'pseudo',
'synthetic',
'inspired','elysian','divine',
'unsung','unknown','obscure',
'no-go',
'good',
'swell','smashing','slap-up','peachy','not bad','nifty','neat','keen','groovy','great','dandy','cracking','corking','bully','bang-up',
'good enough',
'goodish',
'solid',
'superb',
'unspeakable','terrible','painful','dreadful','awful','atrocious','abominable',
'sorry','sad','pitiful','lamentable','distressing','deplorable',
'frightful','fearful',
'hopeless',
'horrid',
'stinky','stinking','shitty','rotten','lousy','icky','crappy',
'ill',
'unskilled','incompetent',
'mediocre',
'naughty',
'negative',
'poor',
'pretty',
'swingeing',
'uncool',
'unfavourable','unfavorable',
'unsuitable',
'good',
'goody-goody',
'white',
'evil',
'monstrous','grievous','flagitious','atrocious',
'bad',
'sinister','dark','black',
'pestiferous','perversive','corruptive',
'unholy','satanic','infernal','hellish','fiendish','diabolical','diabolic','demonic',
'mephistophelian','mephistophelean','diabolical','diabolic','devilish',
'evil-minded',
'good-natured',
'good-humoured','good-humored','amiable',
'ill-natured',
'liverish','dyspeptic','bilious','atrabilious',
'ornery','crotchety','cantankerous',
'churlish',
'disagreeable',
'ugly','surly',
'gainly',
'gracious',
'ungracious',
'churlish',
'fulminant',
'ungrammatical','ill-formed',
'incorrect',
'appreciative',
'unthankful','ungrateful','thankless',
'unappreciative',
'happy',
'unhappy',
'wretched','suffering','miserable',
'unregretting','unregretful',
'solid','firm',
'heartless','hardhearted',
'softhearted','soft-boiled',
'abusive',
'slanderous','libelous','libellous','denigratory','denigrative','denigrating','defamatory','calumnious','calumniatory',
'ruinous','catastrophic',
'prejudicious','prejudicial','detrimental','damaging',
'ill',
'wounding','stabbing',
'harmonious',
'unharmonious','inharmonious',
'healthful',
'parasiticidal','helminthic','anthelmintic','anthelminthic',
'therapeutic','sanative','remedial','healing','curative','alterative',
'salutary','good',
'prophylactic','preventive','preventative',
'unhealthful',
'unhealthy',
'inoperable',
'healthy',
'rosy-cheeked','rosy','rose-cheeked','flushed',
'bouncing',
'firm',
'sound','good',
'whole','hale',
'hearty',
'well-preserved',
'unhealthy',
'angry',
'unsound','unfit','bad',
'bloodshot',
'carbuncular','carbuncled',
'caseous',
'edematous','dropsical',
'inflamed',
'inflammatory',
'proinflammatory','pro-inflammatory',
'sickly','sallow',
'sore-eyed',
'varicose',
'windburnt','windburned',
'dry',
'light-duty',
'worrying','worrisome','troubling','perturbing','disturbing','distressing','distressful',
'harsh',
'low-level',
'nonimitative',
'homological','homologic',
'dishonorable','dishonest',
'shoddy','misleading','deceptive',
'honest',
'honourable','honorable',
'laureate',
'yellow',
'hopeless',
'heartsick','despondent',
'lost','helpless',
'insoluble',
'unconsolable','inconsolable','disconsolate',
'desolate',
'kind','genial',
'antipathetical','antipathetic','antagonistic',
'amicable',
'well-meaning','unthreatening',
'torrid',
'cold',
'polar','icy','glacial','gelid','frigid','arctic',
'unwarmed','unheated',
'cold',
'wintry','icy','glacial','frozen','frosty','frigid',
'inhumane',
'droll',
'joking','jocular','jocose','jesting',
'unhurried',
'of import','important',
'of the essence','essential','crucial','all important','all-important',
'outstanding','great',
'measurable',
'awing','awful','awesome','awe-inspiring','amazing',
'formidable',
'proud','majestic','lofty','gallant',
'disinclined',
'afraid',
'nontransmissible','noncontagious','noncommunicable',
'stygian',
'disingenuous','artful',
'untenanted','unoccupied',
'wounded','hurt',
'torn','mangled','lacerated','lacerate',
'culpable','censurable','blameworthy','blameful','blameable','blamable',
'punishable',
'intellectual',
'highbrowed','highbrow',
'serious','good',
'sophisticated',
'smart','bright',
'undesigned',
'officious','meddling','meddlesome','interfering','busybodied','busy',
'inviting',
'uninviting',
'unsmoothed',
'anisotropic',
'gladsome',
'sad',
'bittersweet',
'mournful','doleful',
'melancholy','melancholic',
'wistful','pensive',
'overjoyed',
'sorrowful',
'bitter',
'heartsick','heartbroken','brokenhearted',
'heartrending','heartbreaking','grievous',
'lugubrious',
'plaintive','mournful',
'woeful','woebegone',
'mirthful','merry','jovial','jolly','jocund','gay',
'joyless',
'unsmiling',
'wrongful',
'gentle',
'rough','harsh',
'legendary',
'unmourned','unlamented',
'larger-than-life','heroic','epic',
'unleaded','leadless',
'gas-tight','airtight','air-tight',
'watertight',
'nonlegal','extralegal',
'baseborn','base',
'liable','apt',
'promising',
'probable','likely',
'untrammelled','untrammeled',
'unlisted',
'unliveable','unlivable',
'unextended',
'lost',
'wasted','squandered',
'stray',
'straying',
'pale',
'hateful',
'likeable','likable',
'unlikeable','unlikable',
'unbeloved','lovelorn','bereft',
'loving',
'amorous','amative',
'taken with','soft on','smitten','potty','infatuated','in love','enamored',
'lucky',
'apotropaic',
'unlucky','luckless',
'jinxed','hexed',
'underage','nonaged','minor',
'secondary',
'minor',
'unmanly','unmanlike','unmanful',
'scarred',
'unbranded',
'unmarried','single',
'unfeminine',
'unsuited','mismated','incompatible','ill-sorted',
'singable',
'unmusical','nonmusical',
'merciful',
'unpitying','ruthless','remorseless','pitiless',
'soft','gentle',
'wicked','terrible','severe',
'smart',
'strong',
'unconcerned',
'unmilitary','nonmilitary',
'unsoldierly',
'relieved','eased','alleviated',
'immobile',
'nonmetallic','nonmetal',
'small','modest',
'usurious','unconscionable','steep','outrageous','extortionate','exorbitant',
'modest',
'unassuming','retiring',
'unmodified',
'unadapted',
'univalent',
'clean-living','clean',
'moralistic',
'righteous',
'incorrupt',
'immoral',
'licit',
'illicit',
'principled',
'inert',
'sitting',
'unnatural',
'undyed','uncolored','unbleached',
'substitute','ersatz',
'factitious',
'simulated','imitation','faux','false','fake',
'supernatural',
'weird','unearthly','uncanny','eldritch',
'transcendental','preternatural','otherworldly','nonnatural',
'incumbent',
'psychoneurotic','neurotic',
'abulic','aboulic',
'nice',
'good',
'pleasant',
'nasty','awful',
'lousy','filthy','dirty',
'grotty',
'mean','hateful',
'ennobling','dignifying',
'untitled','ungentle','ignoble',
'unnatural','abnormal',
'subnormal',
'paranormal',
'pestilent','pernicious','deadly','baneful',
'innocuous',
'unoccupied',
'free',
'offensive',
'repulsive','repugnant','obscene','detestable','abhorrent',
'creepy',
'yucky','wicked','skanky','revolting','repelling','repellent','repellant','loathsome','loathly','foul','distasteful','disgusting','disgustful',
'outrageous','horrific','horrid','hideous',
'scrimy',
'verminous',
'savoury','savory',
'unsavoury','unsavory','offensive',
'odoriferous',
'scurrilous','opprobrious','abusive',
'unoffending','inoffensive',
'unofficial',
'unestablished',
'unrecognized','unrecognised',
'off-street',
'off-line',
'off',
'wrong','untimely','unseasonable','ill-timed',
'inconvenient',
'unopposed',
'unordered','disordered',
'unmethodical',
'unorganized','unorganised',
'wondrous','wonderful','tremendous','terrific','rattling','marvelous','marvellous','howling','grand','fantastic',
'artificial',
'stunning',
'underivative',
'unoriginal',
'stale','moth-eaten','dusty','cold',
'slavish',
'established','conventional',
'nonconformist','nonconforming',
'painful',
'achy','aching',
'agonized','agonised',
'torturous','torturing','torturesome','harrowing','excruciating','agonizing','agonising',
'bitter','biting',
'traumatic',
'painless',
'unpainful','pain-free',
'unpainted',
'unoiled',
'unvarnished','unstained',
'unpaintable',
'unpalatable',
'unsavoury','unsavory','distasteful',
'unimpassioned',
'unforbearing',
'loose',
'unpaved',
'unpeaceful',
'unpeaceable',
'repentant','penitent',
'ruthful','rueful','remorseful','contrite',
'unrepentant','unremorseful','impenitent',
'blind',
'ideal',
'idealized','idealised',
'idyllic',
'imperfect',
'standing',
'irrevokable','irrevocable',
'inadmissible',
'unpermissive',
'perplexed',
'unperplexed',
'impenetrable',
'mothproof','moth-resistant',
'rot-resistant',
'soundproof',
'nonpsychoactive',
'unrelenting','unforgiving','unappeasable','stern','relentless','inexorable','grim',
'unstudied','uncontrived',
'dulcet',
'mortifying','embarrassing',
'harsh',
'hot',
'sore','painful','afflictive',
'ungrateful',
'unhappy',
'displeased',
'admirable',
'charming',
'delightful','delicious',
'fabulous','fab',
'sweet',
'upsetting','disconcerting',
'vexing','maddening','infuriating','exasperating',
'off-putting',
'finished',
'raw',
'unwise','inexpedient',
'imponderable',
'optimistic','affirmative',
'constructive',
'negative',
'neutral',
'impossible',
'unworkable','unfeasible','infeasible','impracticable',
'out',
'uncultivatable','uncultivable',
'unfunctional',
'precise',
'punctilious','meticulous',
'microscopic',
'skillful','nice',
'cretinous',
'delayed',
'secondary',
'nonpublic',
'privileged',
'prolific','fertile','fecund',
'lay',
'juicy','fat',
'remunerative','moneymaking','lucrative',
'idle','dead',
'lean',
'unremunerative',
'proper',
'halal',
'kosher',
'untoward','unseemly','uncomely','unbecoming','indecorous','indecent',
'out-of-the-way',
'wrong','incorrect',
'protected',
'open','exposed',
'naked','defenseless',
'prophylactic','cautionary',
'protecting',
'uppish','too big for one\'s breeches','stuck-up','snotty','snot-nosed','snooty','persnickety','bigheaded',
'self-respecting','self-respectful','dignified',
'vain','swollen-headed','swollen','self-conceited','egotistical','egotistic','conceited',
'modest','mild','meek',
'disarming',
'white',
'unmixed','unmingled','sheer','plain',
'unadulterated',
'modified','limited',
'unimpeachable',
'uneasy','restless',
'unquiet',
'turbulent','tumultuous','troubled','riotous','disruptive',
'squally','squalling',
'lucid','logical','coherent',
'flat-footed',
'notional','imaginary','fanciful',
'unrealistic',
'sound','levelheaded','level-headed','intelligent','healthy',
'twee','prim','niminy-piminy','mincing','dainty',
'gentlemanly','gentlemanlike',
'ladylike',
'superfine','overrefined',
'well-mannered','well-bred',
'ungentlemanly','ungentlemanlike',
'unprocessed',
'unrefined','unprocessed','crude',
'untreated',
'raw',
'untreated',
'unlawful','unconventional','improper',
'unprintable',
'unsuitable','inapplicable',
'unmindful','mindless','forgetful',
'unreplaceable','irreplaceable',
'reputable',
'prestigious','honored','esteemed',
'respectable','honorable','good','estimable',
'well-thought-of','respected',
'notorious','infamous','ill-famed',
'assimilative',
'unsympathetic','closed',
'hostile',
'inconsistent',
'diffident',
'nice','decent',
'answerable',
'unresponsive',
'unexpansive',
'unlaced','unbuttoned',
'unrestrictive',
'emancipative',
'nonrestrictive',
'nonretractile','nonretractable',
'disrespectful','awless','aweless',
'danceable',
'unrhythmical','unrhythmic',
'well-to-do','well-situated','well-off','well-heeled','well-fixed','prosperous','easy','comfortable',
'poverty-stricken','needy','necessitous','indigent','impoverished','destitute',
'pinched','penurious','penniless','in straitened circumstances','impecunious','hard up',
'sumptuous','princely','opulent','luxurious','grand','gilded','deluxe',
'mean','beggarly',
'moneyless',
'insolvent',
'honourable','honorable','ethical',
'wrong',
'vicious','reprehensible','deplorable','criminal','condemnable',
'immoral','base',
'righteous',
'upright','just','good',
'rust-resistant',
'sublime','reverend',
'unsafe','dangerous',
'out',
'opposite',
'sane',
'manic-depressive',
'unstable','unsound','mentally ill',
'paranoid',
'psychotic',
'mordant','grim','black',
'sardonic',
'saturnine',
'unsatisfying','dissatisfactory','disappointing',
'unacceptable',
'unconscientious',
'opened','open',
'obscure','hidden',
'fail-safe',
'shaky','precarious',
'insidious',
'junior',
'subaltern','secondary','petty','lowly','lower-ranking','junior-grade',
'sensational',
'anesthetic','anaesthetic',
'insensitive',
'hygienical','hygienic',
'antiseptic',
'frivolous',
'light-minded','flippant',
'elvish','elfish','elfin',
'broken-down',
'inoperable',
'nonresident',
'nonnomadic',
'vagabond','rootless',
'lecherous',
'shapely',
'well-proportioned',
'unformed',
'sole','exclusive',
'unsheathed','bare',
'colour-blind','color-blind',
'monumental',
'prodigious','portentous',
'false',
'feigned',
'unsized',
'unsorted',
'complete','accomplished',
'virtuoso','masterly','masterful','consummate',
'delicate',
'mean',
'bungled','botched',
'incompetent','fumbling','clumsy','bungling',
'asocial','antisocial',
'unshuttered',
'good-time','convivial',
'unsociable',
'ungregarious',
'unsold',
'wide-eyed','simple','round-eyed','dewy-eyed','childlike',
'sound',
'unsound',
'speculative','risky','high-risk','bad',
'substantial','strong','solid',
'unsound',
'unspecialized','unspecialised',
'generalized','generalised',
'spunky','plucky','feisty',
'listless','dispirited',
'unfluctuating','steady','firm',
'nonstandard',
'nonstandard',
'nonnormative',
'nonstandard',
'unaccepted','unacceptable',
'jerky','jerking','arrhythmic',
'spastic','spasmodic','convulsive',
'stirring','rousing',
'straightarrow',
'unstressed',
'puny',
'unyielding','tenacious','pertinacious','persistent','dour','dogged',
'cross-grained','contrarious',
'insubordinate',
'sufficient',
'insufficient','deficient',
'olympian','majestic',
'upper',
'small','modest','lowly','low','humble',
'indifferent',
'banner',
'brag','boss',
'superb','brilliant',
'splendid','first-class','fantabulous','excellent',
'gilt-edged',
'superlative','sterling','greatest',
'outstanding',
'premium',
'pukka','pucka',
'shining',
'spiffing',
'supreme',
'topping','top-hole','top-flight',
'transcendent','surpassing',
'inferior',
'tinny','sleazy','punk','crummy','chintzy','cheesy','cheap','bum',
'bush-league','bush',
'tawdry','shoddy','cheapjack',
'common','coarse',
'commercial-grade','commercial',
'wretched','woeful','miserable','execrable','deplorable',
'low-grade',
'second-rate','mediocre',
'ropy','ropey',
'stunted','scrubby','scrawny',
'second-class',
'utility-grade','utility',
'unsupported',
'invalidating','disconfirming',
'unsurmountable','insurmountable',
'unconquerable','insuperable',
'stunned','astounded','astonished','astonied','amazed',
'unsusceptible','insusceptible',
'unsuasible','unpersuadable',
'unresponsive',
'unimpressionable',
'dry',
'soured',
'unfermented','sweet','fresh',
'unsuspected',
'unswept',
'unsympathetic',
'buttoned-up',
'short','little',
'wild','untamed',
'esthetic','artistic','aesthetic',
'unpretentious','unostentatious','understated',
'nectarous','ambrosian','ambrosial',
'choice',
'yummy','toothsome','scrumptious','pleasant-tasting','luscious','delicious','delectable',
'temperate',
'unthoughtful','unthinking','unreflective',
'tousled','rumpled','frowzled','dishevelled','disheveled',
'mussy','messy',
'unkempt',
'well-groomed',
'ungroomed',
'flagging','drooping',
'supportable','sufferable','endurable','bearable',
'intolerant',
'intestate',
'rubbery','gristly','cartilaginous',
'untoughened','tender',
'tough',
'virulent','venomous','deadly',
'intractable',
'stubborn','refractory',
'unmanageable','uncontrollable',
'unmalleable',
'troubled',
'worried','upset','disturbed','distressed','disquieted',
'fraught',
'stressed','distressed',
'false',
'mendacious',
'spurious','specious',
'untrue',
'untrusty','untrustworthy',
'shifty','devious',
'tricky','slippery',
'untypical','atypical',
'unorganized','unorganised','nonunionized','nonunionised',
'undivided',
'up','improving',
'serviceable',
'dystopian',
'invalid',
'uncollectible','bad',
'false',
'sophistical','sophistic',
'manky',
'trashy','rubbishy',
'untouchable','unassailable',
'virtuous',
'heinous','flagitious',
'ungodly','sinful','iniquitous',
'unreformable','unredeemable','irredeemable','irreclaimable',
'villainous','nefarious',
'occult',
'unvoluntary','nonvoluntary','involuntary',
'unwilling',
'indefensible',
'unsafe','insecure',
'sheltered',
'unwelcome',
'well',
'sick','ill',
'autistic',
'livery','liverish','bilious',
'consumptive',
'woozy','vertiginous','giddy','dizzy',
'dyspeptic',
'green',
'laid up',
'palsied',
'scrofulous',
'upset',
'salubrious','healthy','good for you',
'unwholesome',
'unwilling',
'battered',
'worthy',
'magna cum laude',
'sacred',
'valued','precious',
'benign',
'immunocompetent',
'immunodeficient',
'anasarcous',
'constitutional',
'deliverable',
'immunocompromised',
'lung-like',
'nonlinguistic',
'aversive',
'nonenzymatic',
'antisemitic','anti-semitic',
'ducal',
'tetanic',
'tetanic',
'unposed']
