...
import regressiontest as rgrs
import classificationtest as clsf
import pylab as pl
...
def classify2():
	...
	thresh = np.median(y_log)
	y_bin = y_log > thresh
	(scores_class,fi) = clsf.getGradientBoostingClassifierCVScore(X,y_bin,completer)
	fi_indexes = fi.argsort()[-5:] # The 5 most important features
	i=1
	print 'Gradient Boosting Classification score : ', scores_class.mean()
	for index in reversed(fi_indexes):
	    print(str(i)+'th component : '+feature_names[index]+' with weight '+str(fi[index]))
	    i=i+1
...
def regress2():
	...
	scores = rgrs.getGradientBoostingRegressorCVScore(X,y_log,completer)
	print 'Gradient Boosting Regression error : ', scores.mean()
